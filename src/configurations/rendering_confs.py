...
class RendererConf:
    ...
    def __post_init__(self):
        assert (
            type(self.samples_per_pixel_per_frame) is int
        ), "samples_per_pixel_per_frame must be an integer"
        ...
        assert (
            type(self.max_volume_bounces) is int
        ), "max_volume_bounces must be an integer"
        assert (
            type(self.subdiv_refinement_level) is int
        ), "subdiv_refinement_level must be an integer"
        assert type(self.headless) is bool, "headless must be a boolean"
        assert type(self.renderer) is str, "renderer must be a string"

        assert (
            self.samples_per_pixel_per_frame > 0
        ), "samples_per_pixel_per_frame must be greater than 0"
        assert self.max_bounces > 0, "max_bounces must be greater than 0"
        assert (
            self.max_specular_transmission_bounces > 0
        ...
        assert (
            self.subdiv_refinement_level >= 0
