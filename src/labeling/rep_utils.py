    ...
    def makeFolder(self) -> None:
        ...
        if self.counter % self.element_per_folder == 0:
            self.current_folder_image = os.path.join(
                self.data_path, str(self.folder_counter)
            )
            self.current_folder_labels = os.path.join(
                self.data_path + "_id_label", str(self.folder_counter)
            )
...
class WriteInstanceData(BaseWriter):
    ...
    def makeFolder(self) -> None:
        ...
        if self.counter % self.element_per_folder == 0:
            ...
            self.current_folder_semantics = os.path.join(
                self.data_path + "_id_semantic", str(self.folder_counter)
            )
            os.makedirs(self.current_folder_image, exist_ok=True)
            os.makedirs(self.current_folder_labels, exist_ok=True)
    ...
    def write(self, data: Dict[str, np.ndarray], **kwargs) -> None:
        ...
        instance_image_data = np.frombuffer(data["data"], dtype=np.uint8).reshape(
            *data["data"].shape, -1
        )
